...
public class ActionBundleFactory {
    ...
    public ActionBundleFactory(BundleContext bundleContext,Component frame, ProgressLayerUI progressLayerUI) {
    ...
    public List<Action> create(final BundleItem bundleItem) {
        ...
        if(bundleItem.isStartReady()) {
            actions.add(new ActionBundle(I18N.tr("Start"),I18N.tr("Activate the selected plugin"),getIcon("execute"), progressLayerUI)
                    .setActionListener(EventHandler.create(ActionListener.class, bundleItem.getBundle(), "start")));
        }
        if(bundleItem.isStopReady()) {
            actions.add(new ActionBundle(I18N.tr("Stop"),I18N.tr("Deactivate the selected plugin"),getIcon("stop"), progressLayerUI)
                    .setActionListener(EventHandler.create(ActionListener.class, bundleItem.getBundle(), "stop")));
        }
        if(bundleItem.isUpdateReady()) {
            actions.add(new ActionBundle(I18N.tr("Update"), I18N.tr("Update the selected plugin"),getIcon("arrow_refresh"), progressLayerUI)
                    .setActionListener(EventHandler.create(ActionListener.class, bundleItem.getBundle(), "update")));
        }
        if(bundleItem.isUninstallReady()) {
            actions.add(new ActionBundle(I18N.tr("Uninstall"), I18N.tr("Remove the selected plugin"),getIcon("uninstall"), progressLayerUI)
                    .setActionListener(EventHandler.create(ActionListener.class, bundleItem.getBundle(), "uninstall")));
        }
        if(bundleItem.isDeployReady()) {
            actions.add(new ActionDeploy(I18N.tr("Download"),I18N.tr("Download the selected plugin"),false,bundleItem.getObrResource(),bundleContext,frame,getIcon("download"), progressLayerUI));
        }
        if(bundleItem.isDeployAndStartReady()) {
            actions.add(new ActionDeploy(I18N.tr("Download & Start"),I18N.tr("Download the selected plugin and start it"),true,bundleItem.getObrResource(),bundleContext,frame,getIcon("download_and_start"), progressLayerUI));
        }
        return actions;
    }
